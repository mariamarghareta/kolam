...
class Monitoringsayur extends CI_Controller {
    public function __construct()
    {
        ...
        $this->load->model('Monitoring_sayur');
    ...
    public function initialization()
    {
        ...
        $this->data["selected_treatment"] = "";
        $this->data["jumlah_obat"] = "";
        $this->data["search_word"] = "";
        $this->data["selected_waktu"] = "";
        $data_count = 10;
        $offset = 1;
        $this->data["arr"] = json_encode($this->Monitoring_sayur->show_all($data_count, $offset, $this->data["search_word"]));
        $this->data["arr_treatment"] = ($this->Obat->show_all_in_stock());
        $this->data["max_data"] = $this->Monitoring_sayur->get_count_all();
    ...
    public function index()
    {
        $this->check_role();
        $this->initialization();
        $this->load->view('monitoring_sayur', $this->data);
    }
    ...
    public function create(){
        ...
        if(!$this->input->post["ph"]){
            unset($_SESSION["list_treatment"]);
        }
        $this->data["state"] = "create";
        $this->load->view('monitoring_sayur_form', $this->data);
    }
    ...
    public function load_data(){
        ...
        $datum = $this->Monitoring_sayur->get($this->data['id'])[0];
        ...
        $this->session->set_tempdata(["list_treatment" => $arrobat], NULL, 5 * 60);
        $this->data["list_treatment"] = json_encode($arrobat);
    }
    ...
    public function load_form_data(){
        ...
        $this->data["tds"] = $this->input->post('tds');
        ...
        $this->form_validation->set_rules('tds', 'TDS', 'required|numeric', array('required' => '%s harus diisi', 'numeric' => '%s harus berupa angka'));
    ...
    public function add_new_data(){
        ...
        {
            $result = $this->Monitoring_sayur->insert($this->data["ph"], $this->data["tds"], $this->data["selected_waktu"], $this->data["keterangan"], $_SESSION['id']);
            if($result  > 0){
                $arrobat = [];
                $is_error = False;
    ...
    public function update_data(){
        ...
        if($this->input->post('write') == "write") {
            if ($this->form_validation->run() != FALSE) {
                $result = $this->Monitoring_sayur->update($this->data["ph"], $this->data["tds"], $this->data["waktu"],
                    $this->data["keterangan"], $this->data["id"], $_SESSION['id']);
                if($result  > 0){
                    $arrobat = [];
                    $is_error = False;
                    if(isset($_SESSION['list_treatment'])){
//                        delete existing bahan penolong
                        $detail = $this->Monitoring_sayur->get_treatment($this->data["id"]);
                        $this->Monitoring_sayur->delete_bahan_penolong($this->data["id"], $_SESSION['id']);
    ...
    public function delete_data(){
        ...
        if($this->input->post('delete') == "delete") {
            $result = $this->Monitoring_sayur->delete($this->data['id'], $_SESSION['id']);
            if($result == 1){
                $detail = $this->Monitoring_sayur->get_treatment($this->data["id"]);
                $res = $this->Monitoring_sayur->delete_bahan_penolong($this->data["id"], $_SESSION['id']);
                for($i=0; $i<sizeof($detail); $i++){
                    $this->Obat->update_live_stok($detail[$i]["obat_id"], $_SESSION['id']);
                }
                if($res){
    ...
    public function addTreatmentList(){
        $this->check_role();
...
//        unset($_SESSION["list_treatment"]);
    ...
    public function removeTreatmentList(){
        $this->check_role();
        $obat_id = $this->input->post('obat_id');
        $arrobat = [];
        if(isset($_SESSION['list_treatment'])){

...
class Masterikan extends CI_Controller {
    ...
    public function initialization()
    {
        ...
        $data_count = 10;
        $offset = 1;
        $this->data["arr"] = json_encode($this->Ikan->show_all($data_count, $offset, $this->data["search_word"]));
    ...
    public function index()
    {
        $this->check_role();
        $this->initialization();
        $this->load->view('masterikan', $this->data);
    }
    ...
    public function update(){
        ...
        $datum = $this->Ikan->get($this->data['id'])[0];
    ...
    public function add_new_data(){
        $this->check_role();
        $this->initialization();
        $this->form_validation->set_rules('tname', 'Nama Ikan', 'required', array('required' => '%s harus diisi'));
        $this->form_validation->set_error_delimiters('<div class="error">', '</div>');

        $this->data['name'] = $this->input->post('tname');
        if ($this->form_validation->run() != FALSE)
        {
            if($this->Ikan->cek_kembar($this->data['name'], -1) == false){
                $this->data['msg'] = "<div id='err_msg' class='alert alert-danger sldown' style='display:none;'>Nama jenis ikan kembar</div>";
            } else {
                $result = $this->Ikan->insert($this->data['name'], $_SESSION['id']);
                if($result == 1){
                    redirect('Masterikan');
                }else{

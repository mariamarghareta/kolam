...
class Pemberian_pakan extends CI_Model
{
    ...
    public function get($id){
        $query = $this->db->select('p.id, round(p.pagi,3) as pagi, round(p.sore, 3) as sore, round(p.malam,3) as malam, t.kode, CONCAT(b.name, k.name) as kolam_name, p.tebar_id, t.tgl_tebar, p.total_ikan, p.biomass, p.size')
